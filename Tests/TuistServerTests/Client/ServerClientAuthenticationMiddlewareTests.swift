...
final class ServerClientAuthenticationMiddlewareTests: TuistUnitTestCase {
    ...
    func test_when_access_and_refresh_tokens_are_expired() async throws {
        let url = URL(string: "https://test.tuist.io")!
        let request = Request(path: "/", method: .get)
        let response = Response(
            statusCode: 200
        ...
        given(serverAuthenticationController)
            ...
            .willReturn(
                .user(
                    ...
                    accessToken: .test(
                        token: "access-token",
                        expiryDate: Date(timeIntervalSince1970: 80)
                    ),
                    refreshToken: .test(
                        token: "refresh-token",
                        expiryDate: Date(timeIntervalSince1970: 85)
                    )
                )
            )
        ...
        await XCTAssertThrowsSpecific(
            ...
            ServerClientAuthenticationError.tokensExpiredOrInvalid
        )
    }
}
